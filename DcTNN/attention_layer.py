...
class BaseAttention(nn.Module):
    ...
    def _select_freqs(self, positions: torch.Tensor, seq_len: int, device: torch.device) -> torch.Tensor:
        ...
        if positions is None:
            if freqs.shape[0] != seq_len:
                raise ValueError(
                    f"RoPE positions required for self-attention subset of length {seq_len}; "
    ...
    def _apply_rope(self, q, k, positions=None):
        freqs = self._select_freqs(positions, q.shape[-2], q.device)
        q = _apply_rotary_emb_single(q, freqs)
        k = _apply_rotary_emb_single(k, freqs)
        return q, k
    ...
    def forward(self, x, attn_mask=None, positions=None):
        ...
        if self.use_rope:
            q, k = self._apply_rope(q, k, positions=positions)
...
class ComplexCrossAttention(nn.Module):
    """
    Complex-valued cross-attention with separate query and key/value inputs.
    Uses the same Hermitian score rule as ComplexMultiHeadAttention.
    """
    def __init__(self, d_model, nhead, dropout=0.0, freqs_cis=None):
        super().__init__()
        self.nhead = nhead
        ...
        self.scale = self.head_dim ** -0.5
        self.dropout_p = dropout
        self.q_proj = nn.Linear(d_model, d_model, dtype=torch.cfloat)
        self.kv_proj = nn.Linear(d_model, 2 * d_model, dtype=torch.cfloat)
        self.proj = nn.Linear(d_model, d_model, dtype=torch.cfloat)
        self.attn_drop = ComplexDropout(dropout)
    ...
    def forward(self, q_input, kv_input, attn_mask=None, q_positions=None, kv_positions=None):
        ...
        x = self._attend(q, k, v).transpose(1, 2).reshape(B, Nq, C)
        return self.proj(x)
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, x, attn_mask=None, positions=None):
        x = x + self.drop(self.attn(self.norm1(x), attn_mask=attn_mask, positions=positions))
...
class TransformerEncoder(nn.Sequential):
    ...
    def forward(self, x, attn_mask=None, positions=None):
        for layer in self:
            x = layer(x, attn_mask=attn_mask, positions=positions)
        return x
...
class CrossAttentionEncoderLayer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward, dropout, activation, layer_norm_eps,
        ...
        if attn_type != "complex":
            raise ValueError(
                "CrossAttentionEncoderLayer only supports attn_type='complex'. "
